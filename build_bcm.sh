#!/usr/bin/env bash
#
# build_bcm.sh - Cross build fakehttp with a BCM-HND toolchain
...
# directories and the final output file name (fakehttp-${ARCH_NAME}).
...
# (e.g. a softcenter/asuswrt plugin bin/ folder), named "fakehttp".
...
Options:
  ...
  --install-dir DIR        Also install the binary as "fakehttp" into DIR (env: INSTALL_DIR)
...
build_fakehttp() {
	...
	echo "== fakehttp: ${arch} (${cross_prefix:-native}) ==" >&2
...
main() {
	...
	local out
	out="$(build_fakehttp "${ARCH_NAME}" "${CROSS_PREFIX}" "${deps_prefix}")"
	...
	if [ -n "${INSTALL_DIR}" ]; then
		mkdir -p "${INSTALL_DIR}"
		install -m 0755 "${out}" "${INSTALL_DIR}/fakehttp"
		echo "installed:"
		file "${INSTALL_DIR}/fakehttp"
	fi
}
