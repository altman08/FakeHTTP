...
static int ipt4_iface_setup(void)
{
    ...
    size_t i;
    int res;
    char *ipt_alliface_src_cmd[] = {"iptables",              "-t",
                                    "mangle",   "-A",         "FAKEHTTP_S",
                                    "-j",       "FAKEHTTP_R", NULL};

    char *ipt_alliface_dst_cmd[] = {"iptables",              "-t",
                                    "mangle",   "-A",         "FAKEHTTP_D",
                                    "-j",       "FAKEHTTP_R", NULL};

    char *ipt_iface_src_cmd[] = {"iptables",              "-t", "mangle",
                                 "-A",       "FAKEHTTP_S", "-i", iface_str,
                                 "-j",       "FAKEHTTP_R", NULL};

    char *ipt_iface_dst_cmd[] = {"iptables",              "-t", "mangle",
...
int fh_ipt4_setup(void)
{
    ...
    int res;
    char *ipt_cmds[][32] = {
        {"iptables", "-t", "mangle", "-N", "FAKEHTTP_S", NULL},

        {"iptables", "-t", "mangle", "-N", "FAKEHTTP_D", NULL},

        {"iptables", "-t", "mangle", "-I", "PREROUTING", "-j",
         "FAKEHTTP_S", NULL},

        {"iptables", "-t", "mangle", "-I", "POSTROUTING", "-j",
         "FAKEHTTP_D", NULL},

        {"iptables", "-t", "mangle", "-N", "FAKEHTTP_R", NULL},

        /*
            exclude local IPs (from source)
        */
        {"iptables", "-t", "mangle", "-A", "FAKEHTTP_S", "-s",
        ...
        */
        {"iptables", "-t", "mangle", "-A", "FAKEHTTP_D", "-d",
         "0.0.0.0/8", "-j", "RETURN", NULL},
        ...
        */
        ...
        {"iptables", "-t", "mangle", "-A", "FAKEHTTP_R", "-m", "mark",
         "--mark", xmark_str, "-j", "RETURN", NULL},

        /*
            send to nfqueue
        */
        {"iptables", "-t", "mangle", "-A", "FAKEHTTP_R", "-p", "tcp",
...
void fh_ipt4_cleanup(void)
{
    size_t i, cnt;
    char *ipt_cmds[][32] = {
        {"iptables", "-t", "mangle", "-F", "FAKEHTTP_R", NULL},

        {"iptables", "-t", "mangle", "-F", "FAKEHTTP_S", NULL},

        {"iptables", "-t", "mangle", "-F", "FAKEHTTP_D", NULL},

        {"iptables", "-t", "mangle", "-D", "PREROUTING", "-j",
         "FAKEHTTP_S", NULL},

        {"iptables", "-t", "mangle", "-D", "POSTROUTING", "-j",
         "FAKEHTTP_D", NULL},

        {"iptables", "-t", "mangle", "-X", "FAKEHTTP_R", NULL},

        {"iptables", "-t", "mangle", "-X", "FAKEHTTP_S", NULL},

        {"iptables", "-t", "mangle", "-X", "FAKEHTTP_D", NULL}};
